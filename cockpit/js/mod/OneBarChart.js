...
OneBarChart.prototype.render = function(data){
	var fixCount = +data.fixCount >= 0 ? data.fixCount : 1;
	var ratio = 100 * (data['current'] - data['lastValue'])/data['lastValue'];
	this._option_.yAxis.data = [(+data['current']).toFixed(fixCount)];
	...
	this._option_.series[0]['label'] = { 
        normal: {
	        ...
	        formatter: function() {
	        	return (ratio > 0 ? '▲ ' : '▼ ') + Math.abs(ratio).toFixed(2)+ '%'; 
	        }
        }
    };
	this._option_.series[0]['markLine']['data'] = [{ xAxis: (+data['lastValue']).toFixed(fixCount) }];
  ...
  //   this._option_.series[0]['markPoint']['data'] = [{ xAxis: (+data['lastValue']).toFixed(fixCount) }];
